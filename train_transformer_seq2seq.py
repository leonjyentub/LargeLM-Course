...
from common.models import TransformerSeq2Seq
...
def main():
    parser = argparse.ArgumentParser(description="Train Transformer Seq2Seq for addition NLP task.")
    parser.add_argument("--data-dir", type=str, default="data/addition")
    parser.add_argument("--out-dir", type=str, default="outputs/transformer_seq2seq")
    ...
    train_loader, val_loader, test_loader = build_seq2seq_loaders(train_df, val_df, test_df, args.batch_size)

    src_vocab_size, tgt_vocab_size = vocab_sizes()
    model = TransformerSeq2Seq(src_vocab_size=src_vocab_size, tgt_vocab_size=tgt_vocab_size)
    logger = create_logger(Path(args.out_dir), name="transformer_seq2seq")
